    ...
    Eigen::VectorXi &met_feature)
{
    ...
    // Rprintf(
    //     "[trav ENTER] v=%d depth=%d | "
    //     "x.size=%d w.size=%d | "
    //     "children_left.size=%d feature.size=%d | "
    //     "leaf_ind.size=%d\n",
    //     v, depth,
    //     x.size(), w.size(),
    ...
    else
    {
        ...
        // protect against bad split_feature
        if (split_feature < 0 || split_feature >= x.size())
        {
            Rcpp::stop("Bad split_feature=%d at node v=%d (x.size=%d)", split_feature, v, x.size());
        }
